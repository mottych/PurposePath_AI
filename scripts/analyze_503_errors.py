        ...
        try:
            ...
            if result["status"] == "Complete":
                ...
                for req_id, logs in request_groups.items():
                    ...
                    has_parse = any(  # noqa: F841
                        "llm_response_parsed" in log["message"]
                        or "llm_response_not_json" in log["message"]
                        for log in logs
                    )
                    ...
                    for log in logs:
                        if "Duration:" in log["message"]:
                            try:
                                duration_str = (
                                    log["message"].split("Duration:")[1].split("ms")[0].strip()
                                )
                    ...
                    if (
                        ...
                        or has_is_final
                        or has_extraction
                        or (duration and duration > 20000)
                    ):
                        print(f"\n{'=' * 80}")
