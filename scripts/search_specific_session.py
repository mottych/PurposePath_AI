                ...
                for event in all_events:
                    ...
                    if "llm_call_completed" in msg:
                        try:
                            ...
                            print(f"[{ts.strftime('%H:%M:%S')}] {status} LLM call completed: {duration:.2f}s")
                    ...
                    if any(keyword in msg.lower() for keyword in ["error", "timeout", "throttl", "failed"]):
                    ...
                    if "REPORT RequestId" in msg and "Duration:" in msg:
                        try:
                            ...
                            print(f"[{ts.strftime('%H:%M:%S')}] {status} Lambda execution: {duration:.2f}s")
                        except:
                            pass
