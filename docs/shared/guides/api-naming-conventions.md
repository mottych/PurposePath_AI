...
This aligns with the deployed runtime OpenAPI/AsyncAPI contracts and current frontend service implementations.
...
- Account runtime contract: logout refresh token supports legacy `refresh_token` while canonical query field is `refreshToken`.
- Traction runtime contract: Python AI payload ingestion may include snake_case input fields; API responses remain camelCase.
...
Most current runtime contracts are camelCase-first. Do not assume snake_case unless the deployed endpoint contract calls it out as legacy/compatibility behavior.

## Validation

### Manual Testing

Use the exact property casing from the deployed runtime OpenAPI contract:
...
- Runtime contract index: `docs/shared/Specifications/README.md`
- Account runtime contract: `https://{api-host}/account/api/v1/openapi/v1.json`
- Traction runtime contract: `https://{api-host}/traction/api/v1/openapi/v1.json`

## Questions?

If you're unsure about a property name:

1. Check the deployed runtime OpenAPI/AsyncAPI contract for the relevant service.
