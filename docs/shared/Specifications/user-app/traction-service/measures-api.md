...
- **List Measure Links:** `GET /measure-links?kpiId={id}`
...
await traction.put(`/measures/${kpiId}/value`, {
  value: 95.50,
...
const kpiDetails = await traction.get<MeasureDetailResponse>(`/measures/${kpiId}`);
console.log(`Linked to ${kpiDetails.data.linkedGoals.length} goals`);

// Delete Measure (soft delete)
await traction.delete(`/measures/${kpiId}`);
```
