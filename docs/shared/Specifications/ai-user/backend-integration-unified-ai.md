...
**Note:** System fields like `id`, `status`, `created_at`, `updated_at`, `conversation_count` are added by the .NET backend when persisting, not generated by the LLM.
...
4. Send insights to .NET backend API for persistence
5. .NET backend adds: `id`, `status`, `created_at`, `updated_at`, `conversation_count`
6. Subsequent page loads fetch from .NET backend (not Python AI service)
