...
Canonical discount-code management for admin now lives under the billing admin surface:
- `GET /admin/api/v1/billing/discount-codes`
- `POST /admin/api/v1/billing/discount-codes`
- `GET /admin/api/v1/billing/discount-codes/{discountCodeId}`
- `PATCH /admin/api/v1/billing/discount-codes/{discountCodeId}`
- `DELETE /admin/api/v1/billing/discount-codes/{discountCodeId}`
- `POST /admin/api/v1/billing/discount-codes/{discountCodeId}/activate`
- `POST /admin/api/v1/billing/discount-codes/{discountCodeId}/deactivate`

Canonical pricing shape:
- `pricing.lines[].adjustmentType`: `percent | amount | override`
- `percentOff` used only when `adjustmentType = percent`
- `amountOff` used only when `adjustmentType = amount`
- `overridePrice` used only when `adjustmentType = override`
