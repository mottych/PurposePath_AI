...
**Request:** **Canonical orchestration envelope only** (not the same as `POST /ai/execute`). Field definitions and rules are authoritative in [`email-insights-api-contract.md`](../ai-api/email-insights-api-contract.md) §4 and §4.7. Topic inputs live in **`activityData`** (equivalent to the old top-level `parameters` object). Example for an onboarding async topic:
...
```json
{
  ...
  "authContext": {
    "serviceToken": "<short-lived-service-token>",
...
- **Canonical API response contract remains nested under `data.business_profile`**:
  ...
  - `business_address`
  - `website`
- **Server-owned scan metadata:** `scan_id` and `captured_at` are generated by backend at response validation time and should be treated as authoritative regardless of model output.
...
1. Call `/ai/execute-async` using the **canonical orchestration envelope** (see [POST /ai/execute-async](#post-aiexecute-async) above) with `topicId: "insights_generation"`, correct `topicCategory` for that topic, and `activityData` for template parameters.
...
```typescript
// 1. Start async job (canonical envelope — see POST /ai/execute-async above)
const { jobId, status } = await coachingClient.post('/ai/execute-async', {
  ...
  authContext: {
    serviceToken,
    expiresAtUtc: serviceTokenExpiresAt,
    issuer: 'PurposePath_Api',
    tokenType: 'service_enrichment'
  }
});
